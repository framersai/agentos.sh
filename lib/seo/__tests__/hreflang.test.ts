...
describe('hreflangAlternates', () => {
  test('homepage emits all 8 locales + x-default; default locale drops prefix', () => {
    const result = hreflangAlternates('/');
    expect(result['en']).toBe('https://agentos.sh/');
    ...
    expect(result['x-default']).toBe('https://agentos.sh/');
  });
  ...
  test('blog post propagates path; default-locale URL is bare', () => {
    const result = hreflangAlternates('/blog/announcing-agentos');
    expect(result['en']).toBe('https://agentos.sh/blog/announcing-agentos/');
    expect(result['zh']).toBe('https://agentos.sh/zh/blog/announcing-agentos/');
    expect(result['x-default']).toBe('https://agentos.sh/blog/announcing-agentos/');
  });

  test('nested path preserved across all locales', () => {
    const result = hreflangAlternates('/legal/privacy');
    expect(result['en']).toBe('https://agentos.sh/legal/privacy/');
